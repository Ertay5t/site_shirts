...
function valid_form(){
    ...
    if(name=="" || name==" ")
    {
        document.getElementById("sp0").innerHTML = "Введите имя"
    }
    else{
        document.getElementById("sp0").innerHTML = "Имя "  + name
    }
...
function rand_tshirt(){
    var arr=["image/futbolka-brendy.jpg", "image/futbolki-blacknwhite.jpg", "image/6184664465.jpg","image/mysterio_gaze.jpg"]; //фото которые хранятся
    var id_img = ["ts_1", "ts_2"] //id элемента
    //БАГИ ИСПРАВИТЬ и сделать сайт по красивее
    for (let i = 0; i<id_img.length; i++) {
        c = document.getElementById(id_img[i]);
        img = randomElement(arr); //получили рандомную фото
